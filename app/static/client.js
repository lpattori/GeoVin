...
function analyze() {
    ...
    if (uploadFiles.length !== 1) alert("Please select an image to be analyzed!");
    ...
    xhr.onload = function(e) {
        if (this.readyState === 4) {
            ...
            txt = "<p>Click on the fire to get the heatmap for the category. Click on the image to restore</p>" ;
            for (i in response) {
                modelos = response [i];
                txt += "<p>" + modelos[0];
                modelo = modelos[1];
        ...
        el("analyze-button").innerHTML = "Analize";
    };
